...
def set_velocity(velocity, s_elev, b_elev, Qv, Vv, dofsVv_x, dofsVv, bed_diff1):
    ...
    #***********************Velocity on rectangular mesh, has to be separated into vertical and horizontal components******
    # Define velocity on 2D rectangular mesh x and y component
    v_new1 = Function(Vv)
    count = 0
...
def readmeshvelocity(fname_vel, nx1, nx2, ny1, ny2, order):
    ...
    """ Read the mesh for the intermediate time step """
...
def write_surface(s_elev, b_elev):
    ...
    """  Write new surface elevation into volume stl  """
    ...
    fp_out.close()
    fp.close()
    print("write surface function closed")

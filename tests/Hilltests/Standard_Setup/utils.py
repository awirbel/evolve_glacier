...
def set_velocity(vel2, s_elev, b_elev, Qv, Vv, dofsVv_x, dofsVv, bed_diff1):
    ...
    for dof, dof_x in zip(dofsVv, dofsVv_x):
        if count % 2 == 0:
            if s_elev(dof_x[0], dof_x[1]) - b_elev(dof_x[0], dof_x[1]) > (bed_diff1 + 1.e-6):
                v_new1.vector()[dof] = vel2(dof_x[0], dof_x[1], 0.0)[0]
            else:
                v_new1.vector()[dof] = 1.e-50

        elif count % 2 != 0:
            if s_elev(dof_x[0], dof_x[1]) - b_elev(dof_x[0], dof_x[1]) > (bed_diff1 + 1.e-6):
                v_new1.vector()[dof] = vel2(dof_x[0], dof_x[1], 0.0)[1]
            else:
                v_new1.vector()[dof] = 1.e-50
        count = count + 1

...
# Variational Form without subdomains
F1 = v * (u - u0) * dx + dt_form * (v *(v_new[0] * grad(u_mid)[0] + v_new[1] * grad(u_mid)[1]) * dx - v_vert*v*dx - f0 * v * dx

# Add continuous interior penalty (CIP) stabi terms
gamma = 0.075
j = 0.5 * gamma * (avg(h)**2) * jump(grad(u_mid), n) * jump(grad(v),n)* dS
F = F1 + j

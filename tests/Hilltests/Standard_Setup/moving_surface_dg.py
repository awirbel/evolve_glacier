"""
    ...
    demo_dg-advection-diffusion.py Copyright (C) 2008 Johan Hake as part of DOLFIN
...
# Initial condition: surface elevation at the beginning of the computations in DG and CG Function Space
=u0_cg = Function(Q3)
u0_cg = s_new
u0 = Function(Q_dg)
u0 = snew_dg
# save_pvd("rmesh_u0", u0, pvd_number=tna)
...
# Upwinding
bn = (dot(v_new, n) + abs(dot(v_new, n)))/2.0

# From dg advection-diffusion FEniCS demo
def a(u,v) :
        # Bilinear form
        a_int = dot(grad(v), kappa*grad(u) - v_new*u)*dx

        a_fac = kappa*(alpha/avg(h))*dot(jump(u, n), jump(v, n))*dS \
                - kappa*dot(avg(grad(u)), jump(v, n))*dS \
                - kappa*dot(jump(u, n), avg(grad(v)))*dS
...
if TS == 'CN':
    ...
    t_ref = 1
    T_ref = 1
# Form for nonlinear variational problem
F = (u - u0)*v*dx(1) + (u - u0)*v*dx(0) + dt_form * (amid -v*f0*dx(1) -v*f0*dx(0) - v_vert*v*dx(1))
...
Smax = interpolate(constraint_u, Q_dg)
...
while flag_stop < 0.5:
    ...
    hice_new = ice_mass(u_cg, s_new_bed, Q3)
    hice_previous = ice_mass(u0_cg, s_new_bed, Q3)
    ...
    hice_change = check_change(s_new, s_new_bed_ini, u_cg, Q3, dofsQ3_x, dofsQ3)
...
hice_solution = ice_thickness(Q3, dofsQ3_x, dofsQ3, u_cg, s_new_bed)
